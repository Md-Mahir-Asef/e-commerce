...
import type { Order, OrderResponse, OrdersResponse, PlaceOrderResponse, OrderStatus } from "@/types/order";
...
export function useOrders() {
    ...
    const getOrderById = useCallback(async (id: number): Promise<Order | null> => {
        try {
            ...
            const errorMessage = err.response?.data?.message || 'Failed to load order';
            toast.error(errorMessage);
            return null;
        }
    }, []);
